...
(** Parse a W3C traceparent header value. Returns [None] if malformed.
    Rejects the all-zero trace-id/span-id values reserved as invalid by W3C.
    Follows the spec's forward-compatibility rule: version ["00"] must be
    exactly the 4 documented fields, but any other version (except the
    reserved-invalid ["ff"]) may carry additional trailing fields for that
    future version's own use — those are ignored, since trace-id/span-id/flags
    still live at the same fixed positions regardless of version. *)
